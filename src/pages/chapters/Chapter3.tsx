...
function InfoSection(): JSX.Element {
	return (
		<Section name={info.sections[0]}>
			<p>
				This section is found in the topmost left section of the character sheet, and records the most basic information
				about your character, including their name, combat class, level, race, and alignment. Races, classes, and
				alignments are discussed in more detail in the
	...
	return (
		<Section name={info.sections[1]}>
			...
			<p>
				...
				each level by one half their strength score. The base value can be bolstered by other effects such as class
			...
			<p>
				...
				event roll testing the martial arts or accuracy skill of the attacking player. Mitigation provides the
				difficulty threshold that determines whether the attack succeeds. Thus, players with high mitigation are hit
				less often and survive longer.
			</p>
			<p>
				Mitigation is composed of two values: dodge chance (DC) and block chance (BC). Dodge chance is initially equal
				to the player’s dexterity attribute, while block chance is determined by the character’s shields and armor. The
				two values are added together to calculate total mitigation. Note that DC is reduced after every attack by the
				attacker's
				<Sub attr='DEX' /> modifier, whether the attack is successful or not. Dodge chance is also reduced to zero (for
				both characters) when a character is grappled.
			</p>
			...
			<p style={{marginLeft: "25px"}}>
				Note: The character sheet is meant to record the base mitigation statistic. Temporary or conditional bonuses
				(such as from potions, spells, or parrying) should not be recorded here.
			</p>
			<p>
				To calculate effective MIT against an attack, add any circumstantial bonuses or penalties to BC and DC to the
...
function AttrsSection(): JSX.Element {
	return (
		<Section name={info.sections[2]}>
			<p>
				The six attributes, <i>Strength</i>, <i>Dexterity</i>, <i>Speed</i>, <i>Intelligence</i>, <i>Instinct</i>, and{" "}
				<i>Charisma</i>, describe the raw physical and mental capabilities of a character. Each attribute has a value
				typically from 0 to 20, with a score of 0 indicating a total absence of the attribute (as with someone who is
				dead or unconscious) while a score of 20 indicating a nearly superhuman level of aptitude. For humans, a score
				of ten indicates an average level of proficiency.
			</p>
			<p style={{marginLeft: "25px"}} id='modifier'>
				Note: In some situations, we do not use the attribute score directly, but the <i>modifier</i>. The modifier for
				an attribute is obtained by subtracting 10 from the attribute score. An average human with 10 for all attribute
				scores would thus have 0 for their modifiers.
			</p>
			<Table
			...
			/>
			<p>
				Each attribute affects multiple aspects of gameplay, meaning that players should care about which stats they
				prioritize and which they neglect. Not every character needs to be well-rounded, but characters with unusual
				ability score distributions should feel somewhat unbalanced to play.
			</p>
			<p>
				If you're filling out your character sheet for the first time, your attributes are going to be determined by
				your racial and class bonuses. Go ahead and add up the totals and record them onto your character sheet, then
				move to the next section. Your attribute scores will increase later on as you level up.
			</p>
		</Section>
	);
}
	...
	return (
		<Section name={info.sections[3]}>
			...
			<p>
				...
				score of 15. If you chose a longsword, you would write:
			</p>
			<pre>Longsword 1d12+3</pre>
			<p>
				The 1d12 is the die used by the weapon, while the +3 comes from your strength, because the longsword is a
				strength weapon. On the other hand, if you chose a katana, you would enter:
			</p>
			<pre>Katana 1d8+5</pre>
			<p>
				Since the katana is a nimble weapon, you want to use whichever is higher of strength and dexterity for damage.
				Since 15 is higher than 13, you would use dexterity.
			</p>
			<p>
				Finally, write down whichever modifiers apply to that weapon. You can refer to <AppendixLink appendix={3} /> for
				more information on weapons and modifiers. If you're fully familiar with your weapon's modifiers, I suggest
				using this space to record the abilities of any magical weapons you equip.
			</p>
...
function SkillsSection(): JSX.Element {
	return (
		<Section name={info.sections[4]}>
			<p>
				Skills are related to attributes in that they are also used for event rolls. Every skill has an underlying
				supporting attribute, whose modifier is added to the roll as with an attribute check. In addition to this,
				players add a bonus from training called a proficiency bonus. The final value of the skill check equals the
				number rolled on the d20 plus the attribute modifier plus the proficency bonus, as follows:
			</p>
			<pre>{strings.skill_check_formula}</pre>
			<p>
				Although every skill depends on at least one attribute, several skills have a secondary supporting attribute
			...
			<p>
				When filling out this section, copy the{" "}
				<ChapterLink chapter={3} target='modifier'>
					modifier
				</ChapterLink>{" "}
				of the relevant attribute under the column labeled "base". Then, record your proficiency bonus (if any) under
				"bonus". Add the two together to calculate your total bonus for that skill. For example, if your character has a
				dexterity score of 13 and proficiency in
			...
			<p>
				There are five levels of proficiency in each skill. Each proficiency level has a corresponding bonus, as
				follows:
			</p>
			<table className='default'>
				<tbody>
			...
			<p>
				Each of these labels indicate that the player has either undertaken training or has an especial weakness in that
				skill. Deficiencies can be overcome and expertise and mastery can be attained through training even after a
				character is made.
			</p>
			<p>
				When players obtain proficiency bonuses to the same skill from multiple sources, they do not combine to advance
				one’s proficiency in that skill to the next level. However, if a player acquires proficiency for the same skill
				from both their race and class, they may take proficiency in a new skill to replace the doubled proficiency.
			</p>
		</Section>
	);
}
...
function AbilitiesSection(): JSX.Element {
	return (
		<Section name={info.sections[5]}>
			<p>
				Abilities are specific skills that confer some benefit to the player, such as spells or feats. Abilities are
				categorized as innate, trained, or rote. Innate abilities are acquired from a player’s class or race (such as
				the barbarian’s rage), and cannot be obtained via training. Abilities learned from trainers are called trained
				abilities, but become rote once their practice has become fully mastered.
			</p>
			<p>
				In this section, write down the name of any abilities you learn, followed by the training slots used by that
				ability. Innate abilities don't need to go here, since they are determined by your class. If an ability is known
				by rote, leave the "training slots" column for that ability blank. If you're filling out a character sheet for a
				level one character, this section will be mostly blank except for any starting spells.
			</p>
		</Section>
	);
}
...
function BackstorySection(): JSX.Element {
	return (
		<Section name={info.sections[6]}>
			<p>
				This section is highly unique to each character, and describes their origins, their motivations for becoming an
				adventurers, personal goals, as well as character strengths or flaws. There's a lot that can go here, so use
				your judgement and flesh out your character as much as you like.
			</p>
...
function InventorySection(): JSX.Element {
	return (
		<Section name={info.sections[7]}>
			<p>
				The final aspect of your character is their inventory, which includes everything they are carrying including
				equipped weapons and armor. If you're filling out your character sheet for the first time, you should refer to
				your class choice to find your initial loadout. When you find additional items during your adventure, you'll add
				them here.
			</p>
			<p>
				Inventory items are categorized as weapons, gear (clothing, armor, and jewelry), tools, and consumables. All of
				these items are stored in your adventurer's pack. If any inventory item is equipped (such as a magic ring or
				amulet) you can put a star or other special mark next to it, to indicate that it is worn. There is no limit to
				the number of magical items you can equip, although you can't gain benefits from wearing multiple items of the
				same type or on the same bodypart. For example, you can't receive the bonuses from two amulets or rings at once,
				even if your character wears them at the same time.
			</p>
...
function RecapSection(): JSX.Element {
	return (
		<Section name={info.sections[8]}>
			<p>
				Now that we've covered the various components of the character sheet, let's walk through the process of creating
			...
			<blockquote>
				...
				human raised on the family farm. (Meaning the true source of his powers remains a mystery.) Arnoch has a strong
				moral compass, but little regard for the law, making him unpredictable and threat to corrupt politicians. As
				Arnoch sets out to find his fortune, he little guesses just what he's heading into... or that he is already
				being watched, on account of his extraordinary powers.
			</blockquote>
			<p>
...
export default function Chapter3(): JSX.Element {
	return (
		<Chapter index={index}>
			<p>
				The character sheet, available <SheetLink>here</SheetLink> is divided into 8 sections:
			</p>
